	...
	<jsp:useBean id="UserSs" scope="session" class="smartit.u1classm.UIDSession"></jsp:useBean>
		<form action="#" method = "post">
			...
			<nav role="navigation">
				<div class = "menu">
						<ul id="main-menu">
							<li><a href = "#">Smart IT</a>
								<ul id = "sub-menu">
									<li><a href="FreeMemoList.jsp?type=김문정&ifcheck=pass">김문정교수님</a></li>
									<li><a href="FreeMemoList.jsp?type=박충식&ifcheck=pass">박충식교수님</a></li>
									<li><a href="FreeMemoList.jsp?type=김재홍&ifcheck=pass">김재홍교수님</a></li>
									<li><a href="FreeMemoList.jsp?type=최제영&ifcheck=pass">최제영교수님</a></li>
								</ul>
							</li>
				...
				<br>
				<table align = "center" border="1" cellpadding = "2" cellspacing = "0" id="table1">
				<tr>
					...
					<!-- 게시판 table -->
						<div class="tableBox">
							<div class="TBox1">
								<div class="t1">
									<h3>자유게시판<a href="FreeMemoList.jsp?type=free&ifcheck=pass">더보기</a></h3>
								</div>
								<hr>
								<table height = "100" border="1">
									...
									try{
										...
										for(int i = 0;i<5;i++){
											%>
											<tr height="20">
												<td height="40"><%=rs.getInt("idx")%>&nbsp;&nbsp;<a href="ViewMemo.jsp?type=nomal&ifpass=pass&idx=<%=rs.getInt("idx")%>"><%=rs.getString("title")%></a></td>
											</tr>
											<%rs.next();
										}
					...
					<th width="200" height="200">
						<div class = "L1">
							...
							<table>
								<tr>
								<td>
									<h5><%=UserSs.getId()+"님 | " %><a href = "UserInfo.jsp">내정보</a><br>
									<%=UserSs.getEmail() %></h5></td>
									<td>
					...
					<%
						try{
							Connection conn = DBUtill.getMySqlConnection();
							String sql = "select * from realtimeuser";
							PreparedStatement pstmt = conn.prepareStatement(sql);
							ResultSet rs = pstmt.executeQuery();
							rs.next();
							%>
							<th rowspan="2" >
							<div class = "scrollRT">
								<table  class = "RTUser"  border = "1" >
									<%
									int i = 0;
									do{
										%>
										<tr height="20">
											<td height="40"><a href="#"><%=rs.getString("id")%></a></td>
										</tr>
										<%
										i++;
										if(i==10)break;
